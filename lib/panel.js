...
/** One-click setup must never chain several full diagnostic timeouts. */
...
export function projectModel(model, pin, meta, hidden = false) {
  ...
  return {
    ...
    // Whether the last probe paid for the `__probe__` harvest. False means the
    // channel list came from the routing reply itself, which is the fast path —
    // surfaced so a quick probe is explainable rather than suspicious.
...
export function createPanel({ control, engine, store }) {
   ...
   *
   * The removal goes through `removeConfigKeys`, not `updateConfig`: a settings
   * `update` merges plain objects recursively and never carries `undefined`, so
   * rewriting the surviving dictionary leaves the deleted key in place — the
   * account row would stay on screen and the delete button would look broken.
   */
  async function accountRemove(payload) {
    ...
    await control.removeConfigKeys('accounts', [name])
    // The active account cannot point at something that no longer exists; that
    // is a separate field, so it still goes through a normal patch.
    if (String(config.activeAccount ?? '') === name) await control.updateConfig({ activeAccount: '' })
  ...
  async function modelProbe(payload, { signal } = {}) {
    const model = modelOf(payload)
    const result = await engine.probe(model, { signal })
    return { model, result: { ...result, error: firstLine(result.error, 300) } }
  }

  /** Test every known channel of one model and record the verdicts. */
  async function modelValidate(payload, { signal } = {}) {
    const model = modelOf(payload)
    ...
    const outcome = await engine.validate(model, { signal })
    return {
      model,
  ...
  async function modelTest(payload, { signal } = {}) {
    const model = modelOf(payload)
  ...
  /**
   * The one-click path: probe, measure every channel, then pin the channels
   * whose strict validation already proved they answered. Nothing usable leaves
   * the model on automatic routing rather than pinned to a dead channel.
   */
  async function setupAuto(payload, { signal } = {}) {
    const model = modelOf(payload)
    ...
    const shape = {
      ...
      verified: false, actual: '', timedOut: false, cancelled: false,
      summary: { ok: 0, limited: 0, bad: 0, auth: 0, unknown: 0 },
    }
    const deadlineMs = Date.now() + AUTO_BUDGET_MS
    const probe = await engine.probe(model, { signal, deadlineMs })
    if (probe.ok !== true) {
    ...
    const validated = await engine.validate(model, { signal, deadlineMs })
    if (validated.cancelled === true || signal?.aborted === true) {
      return {
        ...shape,
        stage: 'cancelled',
        pipeline: String(probe.pipeline ?? ''),
        channels,
        cancelled: true,
        error: 'the automatic setup was cancelled before it changed this model',
    ...
    const preferred = bySpeed(usable)
    const pin = {
    ...
    // A strict validation response already proves both availability and pin
    // adherence. Reusing it avoids a second, identical network call after every
    // successful auto-configuration.
    const verified = preferred
      .map((channel) => validated.results.find((result) => String(result.upstream) === channel))
    ...
    return {
      ...
      timedOut: validated.timedOut === true,
      error: verified !== undefined
        ? ''
        : validated.timedOut === true
          ? 'channel checks reached the time limit before a usable channel answered'
...
export function registerPanel(ctx, { control, engine, store, logger }) {
  ...
  const handle = async (endpoint, payload, options = {}) => {
    ...
    try {
      return { ok: true, value: await route(payload ?? {}, options) }
    } catch (error) {
      return {
        ok: false,
  ...
  const dispose = ctx.inject(['connection'], (connectionCtx) => {
    const registered = connectionCtx.connection.fetch.register({
      ...
      fetch: async (request) => {
        ...
        const result = await handle(String(body?.endpoint ?? ''), body?.payload, { signal: request.signal })
