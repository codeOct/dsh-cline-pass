/**
 * Transport for the Cline Pass gateway and its public catalogs.
 *
 * Everything here is transport-only: it sends the request it is given and
 * returns decoded bytes, with no pinning, retry, or failover policy of its own
 * (that lives in {@link module:dsh-cline-pass/adapter}). Endpoints:
 *
 * - `POST {baseURL}/chat/completions` — the gateway itself (streaming and not)
 * - `GET  {baseURL}/models`            — the gateway's own model catalog
 * - `GET  https://api.cline.bot/api/v1/ai/cline/recommended-models` — the
 *   authoritative subscription list (`clinePass`), no auth required
 * - `GET  https://models.dev/api.json` — the community registry, a backstop
 * - `GET  https://openrouter.ai/api/v1/...` — endpoint detail for direct-pipeline models
 *
