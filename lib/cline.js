...
/** Keep a multi-request public metadata lookup inside its caller's deadline. */
function timeoutBefore(deadlineMs, timeoutMs) {
  if (!Number.isFinite(deadlineMs)) return timeoutMs
  const remaining = deadlineMs - Date.now()
  return remaining > 0 ? Math.max(1, Math.min(timeoutMs, remaining)) : 0
}
 ...
 *
 * The two sources are wildly asymmetric, which is what shapes this function.
 * Measured on this host, the gateway's own recommended-models endpoint answers in
 * ~0.7-3 s with a ~3 KB body, while `models.dev/api.json` is a ~4.9 MB document
 * that has taken 18 s and, on another run, 87 s. The gateway is also the
 * AUTHORITATIVE source: it is the subscription's own list. So the community
 * registry is a best-effort supplement, not an equal peer whose latency sets the
 * refresh time — waiting even a few seconds to re-confirm a list the gateway has
 * already supplied is time the caller spends staring at a spinner.
 *
 * `community: 'await'` keeps the old wait-for-both behavior for a caller that
 * genuinely wants the union in one result.
 *
 * @param options.communityTimeoutMs - budget for the community registry alone.
 * @param options.community - `'skip'` (default when the gateway answered), `'await'`, or `'background'`.
 * @returns `{ models, sources }` — deduplicated ids plus the sources that answered.
 */
export async function fetchOfficialModels({
  signal,
  timeoutMs = 30000,
  communityTimeoutMs = 8000,
  community,
  fetchImpl = fetch,
} = {}) {
  const found = new Set()
  ...
  const readCommunity = async () => {
    try {
      const response = await fetchImpl(MODELS_DEV_URL, { signal: withTimeout(signal, communityTimeoutMs) })
    ...
    } catch { /* unavailable, or slower than its budget */ }
  }
  const accepted = await readRecommended()
  if (!accepted) {
    // Nothing authoritative: the supplement is the only source there is, so its
    // answer is worth waiting for.
    await readCommunity()
  } else if (community === 'await') {
    await readCommunity()
  } else if (community !== 'skip') {
    // Fire and forget: the ids it might add are published on the next refresh,
    // and the caller is not made to wait for a 4.9 MB document. A failure here is
    // contained, and the budget still applies to the in-flight request.
    void readCommunity().catch(() => {})
  }
  // Snapshot both collections: a background supplement keeps writing into the
  // live `found`/`sources` after this returns, and handing the caller those live
  // references would let a later read observe a different result than the one
  // that was returned.
  return { models: [...found].filter((id) => /^cline-pass\/[a-z0-9._-]+$/.test(id)), sources: [...sources] }
}
 ...
 */
export async function resolveOpenRouterSlug(slug, { signal, timeoutMs = 30000, deadlineMs } = {}) {
  const requestTimeout = timeoutBefore(deadlineMs, timeoutMs)
  if (requestTimeout === 0) return null
  try {
    const result = await sendJSON(`${OPENROUTER_API}/models`, { signal, timeoutMs: requestTimeout })
    const ids = (result.json?.data ?? []).map((model) => model?.id).filter((id) => typeof id === 'string')
 ...
 */
export async function openRouterEndpoints(slug, { signal, timeoutMs = 30000, deadlineMs } = {}) {
  const real = await resolveOpenRouterSlug(slug, { signal, timeoutMs, deadlineMs })
  if (real === null) return { slug, endpoints: [] }
  const requestTimeout = timeoutBefore(deadlineMs, timeoutMs)
  if (requestTimeout === 0) return { slug: real, endpoints: [] }
  try {
    const result = await sendJSON(`${OPENROUTER_API}/models/${real}/endpoints`, { signal, timeoutMs: requestTimeout })
