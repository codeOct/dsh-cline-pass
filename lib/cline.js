 ...
 *
 * Rows are per day and per model, so they sum.
 *
 * @returns `{ ok, days, error }` — `days` carries `{ date, costUsd, tokens }`.
 */
export async function fetchDailyUsage({ baseURL, apiKey, userId, startDate, endDate, signal, timeoutMs = 45000 }) {
  const refusal = { ok: false, days: [], error: 'the daily usage endpoint is unavailable' }
  try {
    ...
    const payload = envelope?.data ?? envelope
    const days = (Array.isArray(payload?.items) ? payload.items : []).map((row) => ({
  ...
  try {
    ...
    for (const window of windows.filter((entry) => Number(entry.endMs) - Number(entry.startMs) >= DAY_MS)) {
      ...
      for (const day of daily.days) {
        reading.tokens += day.tokens
        reading.costUsd += day.costUsd
      }
      reading.days = daily.days.length
