...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // The platform seed table publishes the icon set, so this bundle draws the
    // Plugins-page chevron with the very component the host's own cards use.
    ...
    const CSS = [
      ...
      '.cp-model{border:0.5px solid var(--dsw-alias-border-l1);border-radius:8px;padding:8px;display:flex;flex-direction:column;gap:8px}',
      ...
      '.cp-acct-table th{text-align:center;font-weight:400;font-size:11px;color:var(--dsw-alias-label-tertiary);padding:0 0 6px;border-bottom:0.5px solid var(--dsw-alias-border-l2)}',
      '.cp-acct-table td{padding:6px 0;border-bottom:0.5px solid var(--dsw-alias-border-l1);font-size:12px;vertical-align:middle;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
      ...
      '.cp-acct-add,.cp-acct-key{border-top:0.5px solid var(--dsw-alias-border-l1);padding-top:10px;margin-top:2px}',
      ...
      '.cp-history-table th{position:sticky;top:0;z-index:1;background:var(--dsw-alias-bg-layer-1);text-align:center;font-weight:400;font-size:11px;color:var(--dsw-alias-label-tertiary);padding:2px 0 6px;border-bottom:0.5px solid var(--dsw-alias-border-l2)}',
      '.cp-history-table td{padding:6px 0;border-bottom:0.5px solid var(--dsw-alias-border-l1);font-size:12px;vertical-align:middle;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
      '.cp-history-table tbody tr:hover td{background:var(--dsw-alias-bg-layer-2)}',
      '.cp-history-col-dot{width:18px}',
      '.cp-history-col-when{width:66px}',
      '.cp-history-col-upstream{width:118px}',
      '.cp-history-col-load{width:104px}',
      '.cp-history-when{color:var(--dsw-alias-label-tertiary);font-variant-numeric:tabular-nums}',
      '.cp-history-model{font-family:ui-monospace,SFMono-Regular,Menlo,monospace}',
      '.cp-history-load{color:var(--dsw-alias-label-secondary);font-variant-numeric:tabular-nums}',
      ...
      '.cp-usage-group+.cp-usage-group{border-top:0.5px solid var(--dsw-alias-border-l1);padding-top:10px}',
      ...
      '.cp-usage-item{border:0.5px solid var(--dsw-alias-border-l1);border-radius:8px;padding:10px 12px;display:flex;flex-direction:column;gap:8px}',
      ...
      '.cp-usage-track{height:6px;border-radius:999px;background:var(--dsw-alias-bg-layer-2);overflow:hidden}',
    ...
    const ZH = {
      ...
      historyUpstream: '上游',
      historyLoad: '首字 / 用时',
      historyLatencyHint: '首字＝从发出请求到收到第一段内容的时间；用时＝整次请求总耗时。— 表示始终没有收到内容',
    ...
    const EN = {
      ...
      historyLoad: 'first / total',
      historyLatencyHint: 'first = time from request to the first streamed chunk; total = the whole call. A dash means no chunk ever arrived',
    ...
    /**
     * A request's latency as "first chunk / total".
     *
     * The first figure is what a user waits before anything appears, which is
     * what decides whether a model feels usable; the second is what the whole
     * call cost. A request that never produced a chunk — a refusal, a dead
     * stream — shows a dash rather than a zero, because zero would read as
     * "instant" when the truth is "nothing ever came".
     */
    function latency(entry) {
      const total = ms(entry.ms)
      const first = Number(entry.ttft ?? 0)
      return first > 0 ? `${ms(first)} / ${total}` : `— / ${total}`
    }
    ...
    function foldHeader(title) {
      return h('summary', { key: 'sum', className: 'cp-card-title cp-fold' }, [
        h('span', { key: 'l' }, title),
        h(IconChevronDownOutline14, { key: 'chevron', className: 'cp-chevron' }),
      ])
    }
    ...
    function historyRows(t, entry, showAccount) {
      ...
      const row = h('tr', {
        ...
        h('td', { key: 'model', className: 'cp-history-model', title: entry.model }, label),
        h('td', { key: 'upstream' }, h('span', { className: 'cp-history-meta' }, [
        ...
        h('td', { key: 'load', className: 'cp-history-load', title: t('historyLatencyHint') }, latency(entry)),
      ])
        ...
        entries === null
          ...
          : h('div', { key: 'table', className: 'cp-history' }, h('table', { className: 'cp-history-table' }, [
            h('colgroup', { key: 'cols' }, [
              ...
              h('col', { key: 'model' }),
              h('col', { key: 'upstream', className: 'cp-history-col-upstream' }),
              h('col', { key: 'load', className: 'cp-history-col-load' }),
            ]),
            h('thead', { key: 'head' }, h('tr', { key: 'r' }, [
              h('th', { key: 'dot' }, ''),
              h('th', { key: 'when' }, t('historyWhen')),
              h('th', { key: 'model' }, t('historyModel')),
              h('th', { key: 'upstream' }, t('historyUpstream')),
    ...
    /**
     * The Plugins-page card: the same panel, folded away behind a header row.
     *
     * Every other card on that page is a collapsed row carrying a chevron, so an
     * always-open panel reads as the odd one out and pushes the rest off screen.
    ...
    function PluginSettingsCard(props) {
      ...
      const [open, setOpen] = React.useState(false)
      const title = t('title')
      return h('li', { className: `cp-set-card${open ? ' cp-set-card-open' : ''}` }, [
        h('button', {
          key: 'head',
          type: 'button',
    ...
    function apply(ctx) {
      ...
      // 1. The one configuration surface: a card inside the Plugins page, which
      //    already enumerates this namespace. No `settings.section` alongside it
      //    — a second page in the Settings nav would only duplicate this card,
      //    and the nav is for surfaces that are not already listed there.
      //
      //    The card is folded by default, like every other card the tab
      //    dispatches, and it leads the list: the tab dispatches cards in this
      //    slot's registration order, which a keyed slot sorts by `priority`
      //    (default 0) — so a negative value is the supported way to rank above
      //    the host's own cards regardless of activation order.
