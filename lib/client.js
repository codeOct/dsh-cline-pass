...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      // The account switcher. One quota is on screen at a time, so the tabs are
      // the only thing that says a pool exists and which member is being shown.
      '.cp-usage-switch{display:flex;flex-wrap:wrap;gap:6px;margin:10px 0 2px}',
      '.cp-usage-tab{appearance:none;font:inherit;font-size:12px;line-height:1;padding:5px 10px;border-radius:999px;cursor:pointer;border:0.5px solid var(--dsw-alias-border-l2);background:transparent;color:var(--dsw-alias-label-secondary)}',
      '.cp-usage-tab:hover{color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-label-tertiary)}',
      '.cp-usage-tab-on{background:var(--dsw-alias-bg-module-platform);color:var(--dsw-alias-label-primary);font-weight:600;border-color:var(--dsw-alias-label-tertiary)}',
    ...
    class PanelController {
      constructor(ctx) {
        this.ctx = ctx
        this.store = createStore({ status: 'loading', error: null, data: null, busy: null, pending: [], notice: null, action: null })
        this.disposed = false
      ...
      /** Track concurrent actions without allowing one completion to unlock another. */
      begin(label, patch = {}) {
        this.pending.add(label)
        this.publish({ busy: label, pending: [...this.pending], ...patch })
      }

      /** Complete one tracked action and retain any others still in flight. */
      end(label, patch = {}) {
        this.pending.delete(label)
        const pending = [...this.pending]
        this.publish({ busy: pending.at(-1) ?? null, pending, ...patch })
      }

      /** Run one action with busy/notice bookkeeping. */
      async run(label, action) {
        this.begin(label, { notice: null })
        try {
          const value = await action()
          this.end(label, { error: null, action: value ?? this.store.getSnapshot().action })
          return value
        } catch (error) {
          this.end(label, { error: String(error?.message ?? error), action: null })
          return undefined
        }
      }
      ...
      async probeModel(model) {
        const result = await this.run(`probe:${model}`, async () => await this.call('model.probe', { model }))
        await this.refresh()
        if (result !== undefined) {
          this.publish({
      ...
      async validateModel(model) {
        const result = await this.run(`validate:${model}`, async () => await this.call('model.validate', { model }))
        if (result !== undefined) {
          this.applyState(result)
      ...
      async testModel(model, upstreams) {
        const result = await this.run(`test:${model}`, async () => await this.call('model.test', { model, upstreams }))
        await this.refresh()
        if (result !== undefined) {
          this.publish({
      ...
      async setupModel(model) {
        const label = 'setup:' + model
        this.begin(label, { notice: null, error: null })
        try {
          ...
          this.end(label, { error: String(error?.message ?? error) })
          return undefined
        } finally {
          await this.refresh()
        }
    ...
    function ModelRow(props) {
      ...
      const pending = Array.isArray(busy) ? busy : (busy === null ? [] : [busy])
      const working = pending.includes('setup:' + model.id)
      const modelBusy = pending.some((label) => label === 'setup:' + model.id || label === 'probe:' + model.id || label === 'validate:' + model.id || label === 'test:' + model.id)
      const rowDisabled = disabled || modelBusy
      ...
      const action = (key, label, run) => h('button', {
        key, type: 'button', className: 'cp-btn', disabled: rowDisabled, onClick: () => run(model.id),
      }, label)
      return h('div', { className: `cp-model${hidden ? ' cp-model-hidden' : ''}` }, [
        h('div', { key: 'head', className: 'cp-row' }, [
          ...
          h('button', {
            ...
            disabled: rowDisabled, title: t('autoSetupHint'), onClick: () => actions.setupModel(model.id),
        ...
        open ? h('div', { key: 'detail', className: 'cp-panel' }, [
          ...
          model.upstreams.length > 0
            ? h(ChannelPicker, { key: 'channels', t, model, disabled: rowDisabled, onPin: (payload) => actions.pinModel(payload) })
    ...
    function UsageCard(props) {
      ...
      /**
       * Which account's quota is on screen.
       *
       * One account is shown at a time, chosen by clicking its name — a pool of
       * several accounts each with three windows made the card a wall of bars in
       * which no single account's remaining quota could be read. The selection is
       * view state, not configuration: it is deliberately local, so switching the
       * view never writes to the settings document.
       */
      const [selected, setSelected] = React.useState(null)
      const names = accounts.map((entry) => String(entry.account))
      // A removed or renamed account must not leave the card blank; fall back to
      // the first account whenever the selected one is no longer present.
      ...
      /** One account's block: its windows, without repeating the name above them. */
      const accountGroup = (account) => {
        ...
        return h('div', { key: account.account, className: 'cp-usage-group' }, [inner])
      }
      /**
       * The account switcher: one tab per account, the active one marked.
       *
       * Rendered only for a pool. With a single account there is nothing to
       * switch between, and a one-tab strip would just be noise above the bars.
       */
      const switcher = accounts.length > 1
        ? h('div', { key: 'switch', className: 'cp-usage-switch', role: 'tablist' }, accounts.map((account) => {
          const name = String(account.account)
          const on = name === active
          return h('button', {
            key: name,
            type: 'button',
            role: 'tab',
            className: `cp-usage-tab${on ? ' cp-usage-tab-on' : ''}`,
            'aria-selected': on,
            title: `${t('usageAccount')} ${name}`,
            onClick: () => setSelected(name),
          }, name)
        }))
        : null
      const shown = accounts.find((entry) => String(entry.account) === active)
      let body
      ...
      else body = h('div', { key: 'groups', className: 'cp-usage-groups' }, shown === undefined ? [] : [accountGroup(shown)])
    ...
    function Panel(props) {
      ...
      const pending = Array.isArray(state.pending) ? state.pending : (state.busy === null ? [] : [state.busy])
      // Reads can run in parallel, but setting writes and automatic setup merge
      // shared configuration and must stay serialized.
      const writeBusy = pending.some((label) => label.startsWith('setup:') || ['key', 'account', 'pin', 'visibility', 'models'].includes(label))
      const disabled = writeBusy || (data !== null && data.settingsAvailable === false)
      ...
      return h('div', { className: 'cp-panel' }, [
        ...
        data === null || !ready ? null : h(UsageCard, {
          ...
          counting: pending.includes('windows'),
        ...
        data === null || !ready ? null : h(ModelsCard, { key: 'models', t, data, disabled, busy: pending, actions }),
    ...
    function QuickCard(props) {
      ...
      return h('div', { className: 'cp-panel' }, [
        ...
        account === undefined ? null : h(KeyField, {
          ...
          t,
          account,
          disabled: busy || data?.settingsAvailable === false,
          busy,
