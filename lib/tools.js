...
export function registerTools(ctx, { control, engine, store }) {
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          harvested: { type: 'boolean', required: true },
        },
      },
      render: (_args, value) => {
        ...
        return text([
          ...
          value.harvested ? '' : 'channel list came from the routing reply (no harvest round trip)',
    ...
    async execute(args, exec) {
      ...
      if (result.ok !== true) {
        return { ok: false, model, error: firstLine(result.error, 400), ms: Number(result.ms ?? 0), pipeline: '', pinnable: false, canonicalSlug: '', lastProvider: '', upstreams: [], availableProviders: [], tier0: [], harvested: false }
      }
      return {
        ok: true,
        model,
        ...
        harvested: result.harvested === true,
      }
    },
  }))
    ...
    async execute(args, exec) {
      ...
      if (action === 'test') {
        ...
        if (action === 'set') {
          ...
          delete accounts[key]
          // A removal needs the host's unset path op: a settings `update` merges
          // plain objects recursively, so rewriting the survivors leaves the
          // deleted account in the stored dictionary.
          await control.removeConfigKeys('accounts', [key])
          if (readConfig().activeAccount === key) await control.updateConfig({ activeAccount: '' })
