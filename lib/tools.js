      ...
      render: (_args, value) => {
        ...
        return text([
          ...
          ...value.entries.map((entry) => {
            ...
            // First chunk over total, the same pair the panel shows; a dash marks
            // a request that never produced one.
            // First chunk over total, the same pair the panel shows; a dash marks
            // a request that never produced one.
            const latency = `${entry.ttft > 0 ? `${entry.ttft}ms` : '—'}/${entry.ms}ms`
            return `${when} ${entry.model} -> ${entry.provider || '(auto)'}${entry.canonical === '' ? '' : ` [${entry.canonical}]`} ${latency}${entry.stream ? ' stream' : ''} account=${entry.account || '-'}${attempts}${adherence}${warnings}${error}`
          }),
        ].join('\n'))
      },
    },
