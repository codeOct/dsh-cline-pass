/**
 * The model-facing management tools of the Cline Pass provider.
 *
 * These tools are the console the upstream project exposed as a web page,
 * re-expressed the way dsh prefers it: as typed tool calls an agent can make
 * while it works. They read and write the same settings section the provider
 * itself runs from, so a pin written by `cline_pass_pin` takes effect on the
 * very next model call without a restart.
 *
...
export function registerTools(ctx, { control, engine, store }) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Report the state of the built-in Cline Pass provider: the provider route registered on the dsh LLM seam, every configured account with whether its API key is actually stored, the single/round-robin account mode, how many subscription models are exposed, and how many of them carry a pin. Read-only and free — it never calls Cline upstream. Use it first when a Cline Pass model fails, before reaching for probe/test.',
    parameters: {},
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'List the Cline Pass subscription models this provider exposes, with everything known about each one: its routing pipeline (direct = OpenRouter behind, planner = Vercel AI Gateway behind), whether an upstream can be pinned at all, the discovered upstream channel list, the per-channel availability verdicts learned by probe/validate, and the stored pin. Pass `model` to narrow to one id (substring match) and `refresh: true` to rescan Cline\'s official subscription list and adopt any new models. Read-only and free unless `refresh` is set.',
    parameters: {
      model: { type: 'string', description: 'Only return models whose id contains this text, e.g. "glm-5.2".' },
      refresh: { type: 'boolean', description: 'Rescan the official Cline subscription model list first and adopt new ids.' },
    },
    isConcurrencySafe: () => true,
    timeoutMs: 60000,
    output: {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Discover how one Cline Pass model is routed, by firing a single tiny real request at the gateway: it detects the pipeline (direct vs planner), reads back the upstream that actually served it, and harvests the precise list of upstream channels this pipeline can be pinned to. Costs a fraction of a cent and needs a working account. Run it before pinning a model, and again if a pin that used to work stops working — the gateway\'s channel pools change over time.',
    parameters: {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Test every known upstream channel of one model, one tiny real request each, and report an availability verdict per channel: available, rate-limited, not-pinnable, or auth-failed. This is the ground truth behind a pin decision, because a channel that looks plausible can fail when pinned alone. Costs roughly $0.0002 per channel and can take minutes on a model with many channels; probe the model first so its channel list exists. Verdicts are kept in memory and surfaced by cline_pass_models.',
    parameters: {
      model: { type: 'string', required: true, description: 'Model id, e.g. "cline-pass/glm-5.2".' },
    },
    timeoutMs: 1800000,
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Send one small real request through the configured pin chain to verify that pinning actually takes effect, and report which upstream served it plus the per-attempt failover trace (upstream, HTTP status, latency, note). With `upstreams` it temporarily pins that ordered candidate list (first = primary); with `exclude` it drops channels for this call only. Omit both to exercise the model\'s stored pin. Nothing is persisted, so this is the safe way to try a pin before calling cline_pass_pin. Costs a fraction of a cent per attempt.',
    parameters: {
      model: { type: 'string', required: true, description: 'Model id, e.g. "cline-pass/glm-5.2".' },
  ...
  ctx.tools.register(defineTool({
    name: 'cline_pass_pin',
    description: 'Persist the upstream pin of one Cline Pass model in the provider configuration. `upstreams` is the ordered candidate list: the first entry is primary and the rest are failover targets tried in order (pass an empty array to return the model to automatic routing). `exclude` always wins over `upstreams` and, under automatic routing, is compiled into an allow-list so the gateway cannot fall back onto an excluded channel. `pinMode` is "strict" (only these channels) or "preferred" (prefer them, fall back to others). `sort` orders candidates by "cost", "ttft" (fastest first token) or "tps" (highest throughput); pass "none" to clear it. Only the fields you supply change. Verify with cline_pass_test before trusting the result.',
    parameters: {
      model: { type: 'string', required: true, description: 'Model id, e.g. "cline-pass/glm-5.2".' },
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Inspect and manage the Cline Pass account pool. action="list" reports every account and whether its API key is stored. action="add" registers an account, optionally storing the key immediately (pass `key`) or pointing at an existing credential/env name (pass `apiKeyEnv`). action="remove" deletes one account, action="mode" switches between "single" (manual `active` account key) and "roundrobin" (spread requests evenly), action="set" replaces the whole pool from `accounts`, and action="test" performs a real authorization check against one account key. With no accounts configured, one implicit account reads the top-level `apiKeyEnv` credential. Keys are never echoed back — only a masked hint.',
    parameters: {
      action: { type: 'string', required: true, enum: ['list', 'add', 'remove', 'mode', 'set', 'test'], description: 'Operation to perform.' },
      name: { type: 'string', description: 'Account key (letters, digits, dot, dash, underscore), for add/remove/mode/test.' },
      key: { type: 'string', description: 'Cline Pass API key (sk_…), for add/test; stored in the credential store, never in settings.' },
      apiKeyEnv: { type: 'string', description: 'Credential/environment name holding this account key, for add/set.' },
      displayName: { type: 'string', description: 'Human-readable label, for add/set.' },
      enabled: { type: 'boolean', description: 'Whether the account takes part in routing, for add/set.' },
      mode: { type: 'string', enum: ['single', 'roundrobin'], description: 'Pool mode, for mode/set.' },
      active: { type: 'string', description: 'Account key used in single mode, for mode/set.' },
      accounts: {
        type: 'array',
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Read the provider\'s in-process request history (newest first): for every streamed model call the account used, the upstream that served it, the pinned candidates that were attempted, latency, and any error. Read-only and free. Use it to see which upstream really serves traffic, and to spot failover or repeated 429s over time. History is kept in memory only, so it starts empty after a dsh restart.',
    parameters: {
