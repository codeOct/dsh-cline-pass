...
/**
 * Whether this host's Schemastery implements live configuration references.
 *
 * `Schema.prototype.volatile` exists only from `@deepseek-ai/schemastery`
 * 3.18.4, which is what DSH 0.1.7 pins. The stable line (`dsh@0.1.5-rc.3`, and
 * the 0.1.6 alpha pins that resolve `^3.18.2`) ships 3.18.2, where the method is
 * simply absent — calling it throws `TypeError: z.object(...).volatile is not a
 * function` at MODULE EVALUATION, so the whole plugin fails to import and the
 * route never registers. The earlier claim in this file that the declaration is
 * "harmless on 0.1.5 and earlier" was wrong, and this probe is the correction.
 */
const SUPPORTS_VOLATILE = typeof z.object({}).volatile === 'function'

/**
 * Validated plugin configuration (this plugin's settings section).
 *
 * Declared volatile as a whole where the host supports it, the way
 * `llm-pi-ai` declares its `providers` dict: from dsh 0.1.6 the settings service
 * refuses every write to an entry with no volatile field (`Plugin entry "…" has
 * no volatile fields`), so without this the panel's saves — a pin, a hidden
 * model, a stored key's account — are all rejected while its reads keep working,
 * which reads as "the buttons do nothing". Where the host has no volatile
 * concept at all (stable 0.1.5-rc.3) the declaration is skipped: writes there go
 * through `installSection`, whose registered scope is the live source, and an
 * unknown `extra()` key would only risk a schema-validation failure.
 */
...
export const Config = SUPPORTS_VOLATILE ? ConfigSchema.volatile() : ConfigSchema
...
export function apply(ctx, config) {
  /** Detach a value so schema defaulting can never mutate the host's snapshot. */
  ...
  /**
   * Read a configuration source as plain validated values.
   *
   * Three shapes reach this plugin across supported hosts: an ordinary object, a
   * stable callback source, and the volatile reference 0.1.7 passes at runtime.
   * Alpha hosts expose saved settings as deeply frozen snapshots while
   * Schemastery fills defaults in place, so the value is cloned before schema
   * normalization.
   *
   * The schema's own output is read back with `.get()` ONLY when the schema was
   * declared volatile: `Config(...)` returns a plain object on a host without
   * that concept, and calling `.get()` on it would yield `undefined` and silently
   * discard every configured value.
   */
  const materializeConfig = (raw) => {
    const source = typeof raw === 'function' ? raw() : raw
    const value = typeof source?.get === 'function' ? source.get() : source
    const parsed = Config(cloneConfigValue(value ?? {}))
    if (parsed !== null && typeof parsed === 'object' && typeof parsed.get === 'function') return parsed.get()
    return parsed
  }
  /**
   * The live configuration source, for both host generations.
   *
   * DSH exposes a plugin's live configuration in one of two ways, and the plugin
   * has to work under either:
   *
   * - **0.1.7+ (volatile references).** `apply` receives a frozen reference whose
   *   `get()` returns the current snapshot, and the Loader swaps the value inside
   *   it on every settings write. Two consequences: the object handed to `apply`
   *   must never be treated as live (that freezes the plugin on its launch
   *   snapshot, which is what "the buttons do nothing" was), and values must be
   *   read with `config.get()` rather than `config.field`, because a whole-object
   *   `.volatile()` leaves every direct property `undefined`.
   * - **0.1.5/0.1.6 (registered sections).** `settings.installSection(...)` hands
   *   back a live source through its `setSource` hook, and `fiber.config` is an
   *   ordinary resolved object with no `.get()`.
   *
   * The installed source therefore wins when the host provides one, the fiber's
   * volatile reference is next, and the launch value is the last resort (a
   * headless or direct-test invocation).
   */
  let installedSource = undefined
  let launchConfig = materializeConfig(config)
  config = launchConfig
  const store = createStore({ historyLimit: config.historyLimit })
  /**
   * The configuration every consumer reads, memoized by snapshot identity.
   *
   * `get()` returns a *new* frozen plain object per call — the reference object
   * is itself frozen and only its inner value is swapped — so the reference is
   * useless as a cache key: caching on it would return the first snapshot
   * forever, which is exactly the freeze this code removes. The snapshot object
   * stays identical until a write commits a freshly parsed tree, so its identity
   * is the correct key.
   *
   * `memoryPatch` carries writes made when no settings service is composed, so
   * the panel and the tools still change behavior in a headless composition
   * (they just do not survive a restart).
   */
  let memoryPatch = {}
  let liveSnapshot = undefined
  let liveValue = undefined
  const live = () => {
    // A registered section (stable hosts) is authoritative when present.
    if (typeof installedSource === 'function') {
      let fromSection
      try {
        fromSection = installedSource()
      } catch (error) {
        ctx.logger?.warn('[cline-pass] the registered settings source could not be read; keeping the last good value: %s', errorChain(error))
        fromSection = undefined
      }
      if (fromSection !== undefined && fromSection !== null) {
        const merged = Object.keys(memoryPatch).length === 0 ? fromSection : { ...fromSection, ...memoryPatch }
        const key = merged === fromSection ? fromSection : JSON.stringify(memoryPatch)
        if (key === liveSnapshot && liveValue !== undefined) return liveValue
        try {
          const next = materializeConfig(merged)
          liveSnapshot = key
          liveValue = next
          return next
        } catch (error) {
          ctx.logger?.warn('[cline-pass] the registered settings source is not usable; keeping the last good value: %s', errorChain(error))
          return liveValue ?? launchConfig
        }
      }
    }
    const reference = ctx.fiber?.config
    if (reference === undefined || reference === null || typeof reference.get !== 'function') {
      return Object.keys(memoryPatch).length === 0 ? launchConfig : materializeConfig({ ...launchConfig, ...memoryPatch })
    }
    let snapshot
    try {
      snapshot = reference.get()
    } catch (error) {
      ctx.logger?.warn('[cline-pass] live configuration could not be read; keeping the last good value: %s', errorChain(error))
      return liveValue ?? launchConfig
    }
    const merged = Object.keys(memoryPatch).length === 0 ? snapshot : { ...snapshot, ...memoryPatch }
    const key = merged === snapshot ? snapshot : JSON.stringify(memoryPatch)
    if (key === liveSnapshot && liveValue !== undefined) return liveValue
    let next
    try {
      next = materializeConfig(merged)
    } catch (error) {
      ctx.logger?.warn('[cline-pass] live configuration is not usable; keeping the last good value: %s', errorChain(error))
      return liveValue ?? launchConfig
    }
    liveSnapshot = key
    liveValue = next
   ...
   * `update` is the documented write API and the one the host actually exposes;
  ...
  const updateConfig = async (patch) => {
    ...
    // A host that persists writes is responsible for committing the new value
    // into this plugin's reference; dropping the memo means the next read
    // observes it whether that commit already happened or happens next tick.
    liveSnapshot = undefined
    liveValue = undefined
  }

  /**
   * Remove entries from a nested dictionary in this plugin's settings section.
   *
   * A deletion cannot be expressed by `update`. Its merge is RECURSIVE — "plain
   * objects merge recursively, every other value replaces the lower layer
   * wholesale" — and a patch never carries `undefined`. So writing the surviving
   * dictionary back leaves every omitted key in place, which is exactly why
   * deleting an account appeared to do nothing while the panel cheerfully
   * reported success and the row stayed on screen.
   *
   * `mutate` with an `unset` path op is the host's own removal mechanism, and it
   * deletes the member rather than merging over it. It is used only when the
   * host provides it: an older host without `mutate` falls back to `update`,
   * where a removal is at least not made worse.
   *
   * @param namespace - `'accounts'` or another nested dict field.
   * @param keys - dictionary members to delete.
   */
  const removeConfigKeys = async (namespace, keys) => {
    const list = [...new Set((Array.isArray(keys) ? keys : [keys]).map(String).filter((key) => key.length > 0))]
    if (list.length === 0) return
    const settings = ctx.get('settings')
    if (settings === undefined || typeof settings.mutate !== 'function') {
      const next = { ...(live()[namespace] ?? {}) }
      for (const key of list) delete next[key]
      await updateConfig({ [namespace]: next })
      return
    }
    await settings.mutate(DEFAULT_SETTINGS_NS, list.map((key) => ({ op: 'unset', path: [namespace, key] })))
    liveSnapshot = undefined
    liveValue = undefined
  }

  /**
   * Register with the settings service so the live configuration is observable.
   *
   * There are two supported host contracts and both are handled here, because
   * neither is a superset of the other:
   *
   * - **`installSection` (stable 0.1.5/0.1.6).** On these hosts the settings
   *   service owns the resolved section: `fiber.config` is a plain object with
   *   no `.get()`, so `setSource` is the ONLY channel through which a saved
   *   value reaches the running plugin. Skipping this registration is what makes
   *   every write persist into the document and stay invisible to the route.
   * - **`loader/volatile-update` (0.1.7+).** These hosts have no
   *   `installSection`; instead the Loader swaps the snapshot inside the
   *   volatile reference and announces it, so the memo is dropped and `live()`
   *   re-reads the reference.
   *
   * Both are installed when available. Registering the section on a host that
   * also publishes volatile updates is harmless — `live()` prefers whichever it
   * actually receives — and each block is guarded by its own feature test so a
   * host missing either API still activates.
   */
  ctx.inject(['settings'], (settingsCtx) => {
    const settings = settingsCtx.settings
    if (settings === undefined || typeof settings.installSection !== 'function') return
    try {
      settings.installSection(ctx, DEFAULT_SETTINGS_NS, Config, config, {
        setSource: (source) => {
          installedSource = source
          // The section's value replaced whatever we were reading, so the memo
          // must not answer for it any more.
          liveSnapshot = undefined
          liveValue = undefined
        },
        onChange: () => {
      ...
      // A refusal here must not take the route down: on a host that also emits
      // volatile updates the plugin still tracks the reference.
      ctx.logger?.warn('[cline-pass] settings section registration was refused; live values come from the configuration reference: %s', errorChain(error))
    }
  })

  // 0.1.7+ commits a new snapshot into the volatile reference and announces it on
  // the owning fiber's filtered context. Nothing has to be recomputed here —
  // every consumer reads through `live()` — but the memo has to be dropped so
  // the next read observes the new snapshot instead of the cached one.
  ...
  const control = {
    ...
    // A getter, not a snapshot: the panel renders this name, and a settings
    // write has to be visible on the next read like every other field.
    get displayName() { return live().displayName },
