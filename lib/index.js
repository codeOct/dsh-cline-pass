 ...
 * Harness.
 *
 * Nothing external is required: this plugin registers the Cline Pass
 * subscription models on the harness LLM seam itself, so they appear in the
 * model picker and to every agent exactly like a built-in provider. There is no
 * proxy process to run and no separate console to open — the upstream-pinning
 * work the `cline-pass-switcher` project exposed through a web console is
 * registered here as `cline_pass_*` tools.
 *
 * What it registers on the host plane:
 *
 * 1. One LLM provider route (default `cline-pass`) served by
 *    {@link ClinePassAdapter}, whose catalog comes from the settings-backed
 *    `knownModels` list.
 * 2. A configurable-provider directory entry, so the web Models page knows this
 *    settings namespace configures that route.
 * 3. The `cline_pass_*` management tools.
 *
 * Configuration (accounts, account mode, model list, per-model pins) lives in
 * this plugin's `cline-pass` settings section, so it survives restarts, reloads
 * live, and is editable from the Models page. API keys are never stored there:
 * accounts name a credential reference and the key is resolved per request
 * through the credential seam.
 *
 * Routing behavior — the two pipelines, how a pin is written for each, and how
 * the upstream channel list is harvested — reimplements findings published by
 * the MIT-licensed cline-pass-switcher project:
 * https://github.com/munmunjaklin458-afk/cline-pass-switcher
 *
...
export { ClinePassAdapter, DEFAULT_CONTEXT_WINDOW, DEFAULT_MAX_TOKENS, DEFAULT_STREAM_IDLE_TIMEOUT_MS }
...
/**
 * Register the Cline Pass provider on the harness LLM seam and expose its
 * management tools.
 *
