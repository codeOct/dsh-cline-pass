/**
 * The Cline Pass engine: account selection, pinned request chains with
 * failover, upstream probing, validation, and request recording.
 *
 * The engine owns every network call. The LLM adapter builds on it for
 * streaming chat, and the management tools build on it for probe/validate/test,
 * so both apply exactly the same pinning semantics.
 *
 * Failover contract (mirrors the gateway's own capability): a candidate is
 * abandoned and the next one is tried only while *nothing has been delivered
 * yet*. A non-2xx status, a transport failure, or an error payload that arrives
 * before the first content delta all move to the next candidate; once a stream
 * has produced a chunk it is the answer, and a later failure is reported as
 * such rather than silently retried.
 *
