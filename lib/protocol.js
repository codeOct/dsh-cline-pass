 ...
 */
export const BUILD_TAG = 'dshfix-7'
...
/**
 * Read the routing facts out of one gateway response or stream frame.
 *
 * The gateway reports which upstream answered under
 * `provider_metadata.gateway.routing.finalProvider`, but it hangs that object off
 * a DIFFERENT key depending on how the answer was requested:
 *
 * - a buffered answer puts it on `choices[0].message`,
 * - a streamed answer puts it on `choices[0].delta`.
 *
 * Reading only `message` is what made every streaming call — which is every real
 * conversation — look like it carried no routing at all: the adapter then fell
 * back to the channel the pin had *asked* for, so the history column and the
 * channel verdicts reported the configured channel instead of the serving one.
 * A strict pin that the router quietly ignored was therefore indistinguishable
 * from one it honored. Both shapes (and a bare top-level object) are accepted.
 *
 * @param json - raw decoded response body (enveloped or not), or one stream frame.
