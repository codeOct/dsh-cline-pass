/**
 * Cline Pass wire protocol: routing-pipeline detection, upstream-pin injection,
 * failover-candidate expansion, and error classification.
 *
 * Cline Pass subscription models are served behind one of two pipelines, and
 * the pin is written differently for each:
 *
 * - **direct** — OpenRouter behind the gateway. The response carries a
 *   top-level `provider` (display name) and `model` (real OpenRouter id), and
 *   the top-level `provider.only/order/sort` reaches OpenRouter.
 * - **planner** — Vercel AI Gateway behind the gateway. The response carries
 *   `provider_metadata.gateway.routing` (`canonicalSlug`, `finalProvider`,
 *   `fallbacksAvailable`), and only `providerOptions.gateway.only/order/sort`
 *   survives; the top-level form is discarded before routing.
 *
 * Every function here is pure and synchronous so it can be unit-tested without
 * a network. The routing behavior reimplements findings published by the
 * cline-pass-switcher project (MIT): https://github.com/munmunjaklin458-afk/cline-pass-switcher
 *
 ...
 *
 * Excludes are compiled into an `only` allow-list because the gateway silently
 * ignores the exclude/ignore fields: strict pinning is `only: [upstream]`,
 * preferred pinning is `order: [upstream, ...rest]` plus the allow-list so the
 * gateway cannot fall back onto an excluded channel.
 *
 * A model whose pipeline is still unknown gets both spellings injected — each
 * pipeline ignores the other one.
 *
 * @param body - the outgoing request body (not mutated).
 * @param meta - the model's discovered metadata (pipeline, upstreams).
 * @param attempt - one entry from {@link buildAttempts}.
 * @returns a new body with the pin applied.
 */
export function injectPrefs(body, meta, attempt) {
  const next = { ...body }
  const { upstream = null, orderRest = [], excludeList = [], strict = true, sort = null } = attempt ?? {}
 ...
 *
 * The probe channels a deliberately impossible `only` value, so the router
 * fails before any token is spent and names every provider it could have used:
 * `metadata.available_providers` for OpenRouter, an "Available providers are:"
 * sentence for the Vercel AI Gateway.
 *
 * @param message - the gateway's error text.
 * @param pipeline - `direct`, `planner`, or null when unknown.
