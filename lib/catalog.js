/**
 * Static model metadata for the Cline Pass catalog, plus the merge helper that
 * turns a caller's overrides into the exact metadata the LLM seam validates.
 *
 * Context capacity and output caps are provider facts, not preferences: the
 * harness sizes its prompt budget against `context.contextWindow`, so an
 * understated window silently makes the agent truncate history it could have
 * kept. The values below are the ones the public catalog publishes for the
 * Cline Pass subscription (fetched from models.dev, whose numbers agree with
 * the AI Gateway's own `/v1/models` for the shared slugs).
 *
 * Reasoning vocabulary is NOT per model: the gateway behind Cline Pass rejects
 * anything outside `none|minimal|low|medium|high|xhigh|max`, so one list serves
 * every reasoning-capable model and `off` is deliberately absent — the gateway
 * answers `reasoning_effort: "off"` with HTTP 400.
 *
 ...
 *
 * Precedence per field is: the configured override, then the published
 * catalog, then the caller's route-wide fallback. `provider` and `id` are
 * echoed back because the seam rejects any other spelling.
 *
 * @param provider - the registered route owning this model.
 * @param model - exact model id.
 * @param override - per-model configuration for this id, when present.
 * @param fallback - route-wide defaults from the plugin configuration.
